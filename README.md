...
It is important to note that this is forecast/model-based weather data for the general location, similar to a weather app. It is **not** a physical weather sensor installed on the selected field. If Open-Meteo cannot be reached or returns no usable data, TerraWise marks weather data as unavailable rather than substituting a made-up value.
...
- **Weather failure:** if Open-Meteo cannot be reached or returns unusable data, `weather_available` is returned as `false`, and temperature, humidity, and rainfall are returned as `null`.
...
- The system depends on three external services (Copernicus/Sentinel Hub, Open-Meteo, and Groq); if any of them is down, misconfigured, or rate-limited, the corresponding part of the analysis will be marked unavailable.
